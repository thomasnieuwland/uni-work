 ...
 * Usage and implementation: this script searches the database for the latest nth bugs that have NOT been assigned to developers, sorted by creation date , then returns an array of n bugs formatted as a JSON string. This script takes a POST variable "n" representing the number of newest bugs to display.
 *
 * POST INPUT: n
 */
...
g++ -std=c++11 -Wall -I/usr/include/cppconn -I/usr/include/json -o web/cgi-bin/getUnassigned.cgi cpp/getUnassigned.cpp -L/usr/lib -lmysqlcppconn -lcgicc
*/
...
int main(void)
{
	...
	try {
		...
		prepStmt = conn->prepareStatement( "SELECT * FROM `bug` WHERE `assigned_to` = 'nobody' ORDER BY `creation_ts` DESC LIMIT ?" );
