 ...
 * Filename: alterUserRep.cpp
 * Usage and implementation: this script allows a user with sufficient privileges to give +1 or -1 reputation to another user.
 *
 * POST inputs: id/username/displayname, vote
 */
...
g++ -std=c++11 -Wall -I/usr/include/cppconn -I/usr/include/json -o web/cgi-bin/alterUserRep.cgi cpp/alterUserRep.cpp -L/usr/lib -lmysqlcppconn -lcgicc
*/
...
int main(void)
{
	...
	try {
		...
		else
		{
			...
			{
				retJ["status"] = "failure";
				retJ["message"] = "insufficient user privileges (you need to be an admin or reviewer to use this function)";
			}
