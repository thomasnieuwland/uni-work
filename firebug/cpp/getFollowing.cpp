 ...
 * Date of Creation: 24/10/2016
 * Filename: getFollowing.cpp
 * Usage and implementation:  returns all bugs that the user is following. takes a POST input of "n" which is the maximum number of results to return.
 */
...
g++ -std=c++11 -Wall -I/usr/include/cppconn -I/usr/include/json -o web/cgi-bin/getFollowing.cgi cpp/getFollowing.cpp -L/usr/lib -lmysqlcppconn -lcgicc
*/
...
int main(void)
{
	...
	try {
		...
		else
		{
			...
			prepStmt2 = conn->prepareStatement( "SELECT * FROM bug t1 JOIN following t2 ON t1.bug_id = t2.bug_id WHERE t2.username = ? LIMIT ?" );
			...
			{
				retJ["status"] = "not found";
				retJ["message"] = "You aren't following any bugs";
