 ...
 * Date of Creation: 12/10/2016
 * Filename: createComment.cpp
 * Usage and implementation: this script takes a bug id as input and allows a user with sufficient privileges to upload an attachment. The script checks user permission by reading a user id from the cookie. If the cookie is empty, or the id does not exist in the database (ie: deleted user), then the script will not let the user create an attachment. The script returns a success or failure message formatted as JSON.
 *
 * POST inputs: bug_id, filename, desc
 */
...
g++ -std=c++11 -Wall -I/usr/include/cppconn -I/usr/include/json -o web/cgi-bin/createAttachment.cgi cpp/createAttachment.cpp -L/usr/lib -lmysqlcppconn -lcgicc
*/
...
int main(void)
{
	...
	try {
		...
		std::string file_path = cgi("filename");
		...
		else
		{
			...
			if (res->getInt("isAdmin") == 1 ||res->getInt("isDeveloper") == 1) // Allow everyone with correct user permissions to create comment
			{
				sql::PreparedStatement *prepStmt3;
	
				// Add the new bug to the database
				prepStmt3 = conn->prepareStatement( "INSERT INTO `attachments` (`bug_id`, `attacher`, date, `filename`, `desc`) VALUES (?, ?, NOW(), ?, ?)" );
				prepStmt3->setString(1, bug_id);
				prepStmt3->setString(2, res->getString("displayname"));
				prepStmt3->setString(3, file_path);
				prepStmt3->setString(4, desc);
                ...
                prepStmtFollowCheck->setString(1, bug_id);
