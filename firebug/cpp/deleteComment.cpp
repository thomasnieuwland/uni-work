 ...
 * Filename: deleteComment.cpp
 * Usage and implementation: this script allows the creater of an comment or an admin to delete the relevent comment on a bug report.
 * POST inputs: commentid
 */
...
g++ -std=c++11 -Wall -I/usr/include/cppconn -I/usr/include/json -o web/cgi-bin/deleteComment.cgi cpp/deleteComment.cpp -L/usr/lib -lmysqlcppconn -lcgicc
*/
...
int main(void)
{
	...
	try {
		...
		prepStmt2 = conn->prepareStatement( "SELECT who FROM comments WHERE commentid=?" );
		...
		if (!res->next() || !res2->next() || !res3->next())
		{
			...
			retJ["status"] = "failure";
			retJ["message"] = "insufficient user privileges, incorrect userID or incorrect commentid";
		}
		else
		{
			// User has been found, check for appropriate user privileges
			int userRole = res->getInt("isAdmin");
			std::string commentOwner = res2->getString("who");
			std::string userDisplayName = res3->getString("displayname");
			if (userRole == 1 || commentOwner == userDisplayName) // Allow admins and the creator to edit bug
			{
				sql::PreparedStatement *prepStmt4;
				prepStmt4 = conn->prepareStatement( "DELETE FROM comments WHERE commentid=?" );
