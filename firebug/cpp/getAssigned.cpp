 ...
 * Filename: getAssigned.cpp
 * Usage and implementation:  returns all bugs assigned to the user who accesses the script
 */
...
g++ -std=c++11 -Wall -I/usr/include/cppconn -I/usr/include/json -o web/cgi-bin/getAssigned.cgi cpp/getAssigned.cpp -L/usr/lib -lmysqlcppconn -lcgicc
*/
...
int main(void)
{
	...
	try {
		...
		else
		{
			...
			prepStmt2 = conn->prepareStatement( "SELECT * FROM bug WHERE assigned_to = ? LIMIT ?" );
			...
			{
				retJ["status"] = "not found";
				retJ["message"] = "No bugs assigned to you.";
